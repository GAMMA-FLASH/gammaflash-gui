...
# Author: Bernd L�hlein <bernd.loehlein@netways.de>
...
unless ( $opt_host && ( $opt_sensor || $opt_contact ) ) {
	...
	if (defined $opt_sensor) {
		...
		if ($device =~ m/Poseidon/i) {
			...
			$sensor_tree_id = $enterprise.".4.9.3.1.8";
		...
		if ($device =~ m/Poseidon/i) {
			...
			push(@oids, $enterprise.'.4.9.3.1.2.'.$sensor_id);			# STE2-MIB::sensName
			push(@oids, $enterprise.'.4.9.3.1.3.'.$sensor_id);			# STE2-MIB::sensState
			push(@oids, $enterprise.'.4.9.3.1.4.'.$sensor_id);			# STE2-MIB::sensString
			push(@oids, $enterprise.'.4.9.3.1.5.'.$sensor_id);			# STE2-MIB::sensValue
		} elsif ($device =~ m/STE/i) {
			push(@oids, $enterprise.'.4.1.3.1.2.'.$sensor_id);			# STE-MIB::sensName
			push(@oids, $enterprise.'.4.1.3.1.3.'.$sensor_id);			# STE-MIB::sensState
			push(@oids, $enterprise.'.4.1.3.1.4.'.$sensor_id);			# STE-MIB::sensString
			push(@oids, $enterprise.'.4.1.3.1.5.'.$sensor_id);			# STE-MIB::sensValue
		} else {
			print "ERROR: device not supported";
			$session->close();
			exit $states{UNKNOWN};
	...
	print $output."\n";
	#my $filename = "weather_station_temp.txt";
...
Written by Michael Streb <michael.streb@netways.de>, Bernd L�hlein <bernd.loehlein@netways.de>
